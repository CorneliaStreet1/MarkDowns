...
![image-20220415175313325](https://raw.githubusercontent.com/CorneliaStreet1/PictureBed/master/202204151753826.png)
...
- 设$L$是某个NFA$M_N = (Q,T,\delta,q_0,F)$的语言，则存在DFA，能够接收语言$L$。
  - 定义：$M_D = (Q_D,T,\delta_d, [q0], F_D)$，其中$Q_D = 2^Q$。人话：构造DFA，输入集和NFA一样，状态集是由NFA的状态集作为元素的集合。
...
- 对任何$q \in Q$定义：
  ...
  - $\delta^{'}(q,wa) = ε-CLOUSURE(P),P = \{p| r \in \delta^{'}(q,w) \and p \in \delta(r,a) \}$。
    - 也就是$q$先接收$w$，到达中间状态集$R$，再从$R$的每一个状态初发接收单个字符$a$到达实际的后继状态集，再对这个状态集求闭包。
- <img src="C:/Users/jiangyiqing/AppData/Roaming/Typora/typora-user-images/image-20220419114340945.png" alt="image-20220419114340945" style="zoom: 33%;" />
...
- 有时简记为$LM$。
- 就是两个语言的集合的笛卡尔积，$LM$就是$L$中的每一个语言和$M$中的每一个语言进行拼接得到的字符串的集合。
...
<img src="C:/Users/jiangyiqing/AppData/Roaming/Typora/typora-user-images/image-20220420134059258.png" alt="image-20220420134059258" style="zoom:33%;" />
...
- 翻译成人话就是，从初始状态接受字符串$w$，然后最终能转移掉终态$q$，就是自动机能接收的字符串。
...
- 定义：
  - 文法$G$上的一个句子能够有多种得到它的推导路径，就说明这个文法是二义的。
  - 对于消除文法的二义性，无一般的算法。
